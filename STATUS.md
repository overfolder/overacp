...
| Crate | State |
|---|---|
| `overloop` | Migrated to the broker wire shape. Depends on `overacp-protocol` for method-name constants and for payload types (`TextDelta`, `Activity`, `TurnEndParams`, `QuotaUpdateRequest`, `SessionMessageParams`) — outbound notifications are constructed from the typed structs and inbound `session/message` frames are deserialized into `SessionMessageParams` before being converted to the LLM-facing `Message` shape. (`llm::Message` stays local because it carries typed `tool_calls: Vec<ToolCall>` that the LLM client needs; the bridge to the wire happens via a serde round-trip inside `AcpClient::turn_end`.) `AcpClient` now speaks: single cold-start `initialize` (no more per-message re-init), push-based `next_push()` that blocks on stdin until a `session/message` or `session/cancel` notification arrives, fire-and-forget `turn_end` notification, plus the streaming / heartbeat / quota methods from before. `agentic_loop::run` no longer polls for new messages inside the turn — the outer loop in `main.rs` owns message delivery. `poll/newMessages` and `turn/save` are gone. Tool-call streaming (`stream/toolCall` before every invocation, `stream/toolResult` after, both echoing the same `id` so SSE consumers can pair them) is now emitted by `agentic_loop::run`; failing tool invocations set `is_error = true` on the result frame. Optional Sentry error reporting landed behind cargo feature `sentry` (off by default, zero overhead when unused). New env vars: `OVERLOOP_AGENT_NAME` (process-local fleet identity, attached to the root tracing span and to Sentry events as a tag), `SENTRY_DSN`, `SENTRY_ENVIRONMENT` (default `local`), `SENTRY_TRACES_SAMPLE_RATE` (default `0.1`). Auto-compaction: threshold-triggered LLM summarization (default 80% of 128K context window, keeps last 10 messages, max 3 rounds per session). After compaction, emits `context/compacted` notification carrying the surviving canonical messages and a prose summary; `turn/end` no longer carries messages (deprecated). Configurable via `CONTEXT_WINDOW`, `COMPACTION_THRESHOLD`, `COMPACTION_KEEP_RECENT`, `MAX_COMPACTIONS`. See [`docs/design/context-management.md`](./docs/design/context-management.md). |
...
| `overacp-protocol` | Landed as a workspace member. Carries the canonical `Claims` shape (`{sub, role, user?, exp, iss}` — matches the broker's `server/src/auth.rs`), method-name constants (including `TURN_END`, `CONTEXT_COMPACTED`, `SESSION_CANCEL`, `STREAM_TOOL_CALL`, `STREAM_TOOL_RESULT`), and payload types for every request / notification on the tunnel (`InitializeResponse`, `SessionMessageParams`, `SessionCancelParams`, `TurnEndParams`, `ContextCompactedParams`, stream params, quota params). JWT helpers (`mint_token`, `validate_token`, `peek_claims_unverified`) sit on top of `jsonwebtoken`. Pure types, no tokio. 23 unit + fixture tests (10 JWT round-trips, 13 wire payload round-trips including a `Content::Blocks` multimodal fixture). Consumed by `overloop` as of Phase 1; `overacp-agent` and `overacp-server` follow in later phases. |
| `overacp-agent` | Supervisor + stdio bridge landed (cherry-picked from `feat/0.2-protocol-and-0.3-agent` commits `5e57978` + `e001b7d`). Crate modules: `config` (env-var config: `OVERACP_TOKEN`, `OVERACP_SERVER_URL`, `OVERACP_WORKSPACE`, `OVERACP_AGENT_BINARY`), `tunnel` (tokio-tungstenite WebSocket client with exponential backoff 1s → 30s and retry-forever semantics), `bridge` (content-agnostic byte pump: WS text frames ↔ child stdio, no JSON parsing, runs under `tokio::select!` so either side closing terminates the pair), `process` (child spawn helpers with stdin/stdout piped and stderr inherited, including graceful wait + kill-and-reap on drop), `adapter` (`AgentAdapter` trait + `LoopAdapter` impl — the only adapter in 0.3), `workspace` (`WorkspaceSync` trait + `NoopSync` default), `run` (top-level orchestration: pull workspace, peek JWT `sub` for the tunnel URL, connect with backoff, spawn child, pump bridge, reap child, push workspace). 25 unit tests: tunnel URL rewriting (http→ws, https→wss, ws passthrough, port preservation), `LoopAdapter::command` env-var / reusability, backoff schedule + cap, connect error paths (unreachable host, malformed URL), `process::{spawn, write_line, read_line}` with newline handling + EOF + missing-binary error, `NoopSync` pull/push, `bridge::{ws_to_stdin, stdout_to_ws}` over generic in-memory streams (text frame forwarding, Close frame → TunnelClosed, ping/binary skip, ws-read error, EOF → ProcessExited, sink-failure → Error), and `run::reap_child` for the AlreadyExited + Killed branches. Plus a hermetic integration test `e2e_supervisor_loop.rs` that spins up the broker in-process, a wiremock mock LLM, and launches a real `overacp-agent` subprocess hosting a real `overloop` child — POSTs a user message via REST, subscribes to SSE, and asserts a `turn/end` frame with nonzero usage arrives within 30s. Planned `claude-code` and `codex` adapters remain deferred per TODO § 0.3. |
