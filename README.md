...
> **Status:** early. The reference agent (`overloop`) ships today; the
> protocol/server/agent crates are being extracted from Overfolder.

## Crates

- [`loop/`](./loop) — `overloop`, the reference agent. A minimal agentic
...
The reference agent is at `target/release/overloop`. It expects to be
